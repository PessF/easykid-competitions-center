    ...
    <div x-data="{
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-blue-500/10 text-blue-400 rounded-xl flex items-center justify-center shadow-sm shrink-0 border border-blue-500/20">
                    <i class="fas fa-trophy text-xl"></i>
                </div>
                <div>
                    <h1 class="text-2xl font-semibold text-white tracking-tight">จัดการการแข่งขัน</h1>
                    <p class="text-sm text-gray-400 mt-1 font-normal">สร้างและบริหารจัดการรายการแข่งขันหลักทั้งหมดในระบบ</p>
                </div>
            </div>
            <button @click="$dispatch('open-modal', 'add-competition'); initMap('map-add', 'add_lat', 'add_lng')"
                class="group px-6 py-3 bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white text-sm font-semibold rounded-xl transition-all shadow-md flex items-center justify-center gap-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @forelse($competitions as $comp)
                <div class="bg-[#121212] border border-white/5 rounded-2xl shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.5)] hover:border-white/10 transition-all duration-300 overflow-hidden flex flex-col group">

                    {{-- Image Banner & Status --}}
                    <div class="relative aspect-[21/9] sm:aspect-video bg-[#0a0a0a] overflow-hidden border-b border-white/5 shrink-0">
                        @if ($comp->banner_url)
                        ...
                        <div class="absolute top-4 right-4 z-20 flex flex-col items-end gap-2">
                            @php
                                ...
                                // 🚀 ระบบ Status Badges โทน Dark Mode
                                $statusConfig = [
                                    'draft' => ['bg' => 'bg-white/10 text-gray-300 border-white/20 backdrop-blur-sm', 'icon' => 'fa-file-alt', 'label' => 'ร่างงาน (ซ่อน)'],
                                    'published' => ['bg' => 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30', 'icon' => 'fa-globe', 'label' => 'เผยแพร่แล้ว'],
                                    'coming_soon' => ['bg' => 'bg-amber-500/20 text-amber-400 border-amber-500/30', 'icon' => 'fa-clock', 'label' => 'เร็วๆ นี้'],
                                    'open' => ['bg' => 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30 animate-pulse', 'icon' => 'fa-door-open', 'label' => 'เปิดรับสมัคร'],
                                    'registration_closed' => ['bg' => 'bg-orange-500/20 text-orange-400 border-orange-500/30', 'icon' => 'fa-lock', 'label' => 'ปิดรับสมัคร'],
                                    'ongoing' => ['bg' => 'bg-blue-500/20 text-blue-400 border-blue-500/30', 'icon' => 'fa-play-circle', 'label' => 'กำลังแข่ง'],
                                    'ended' => ['bg' => 'bg-[#1a1a1a] text-gray-500 border-white/5', 'icon' => 'fa-flag-checkered', 'label' => 'จบงานแล้ว'],
                                    'cancelled' => ['bg' => 'bg-red-500/20 text-red-400 border-red-500/30', 'icon' => 'fa-ban', 'label' => 'ถูกยกเลิก'],
                                ];
                            ...
                            <span class="{{ $config['bg'] }} text-[11px] font-semibold px-3 py-1.5 rounded-lg flex items-center gap-1.5 border">
                                <i class="fas {{ $config['icon'] }}"></i> {{ $config['label'] }}
                            </span>

                            @if ($showWarning)
                                <span class="bg-black/70 backdrop-blur-md text-[10px] font-semibold px-2.5 py-1 rounded-lg border flex items-center gap-1.5 {{ $isUrgent ? 'text-red-400 animate-pulse border-red-500/50' : 'text-amber-400 border-white/10' }}">
                    ...
                    {{-- Content --}}
                    <div class="p-6 flex-1 flex flex-col z-20 -mt-8">
                        <h3 class="text-xl font-semibold text-white mb-2 line-clamp-2 leading-snug drop-shadow-md" title="{{ $comp->name }}">
                            {{ $comp->name }}
                        </h3>

                        @if ($comp->location)
                            <a href="https://www.google.com/maps/search/?api=1&query={{ $comp->latitude }},{{ $comp->longitude }}" target="_blank"
                                class="inline-flex items-start text-sm text-gray-400 hover:text-blue-400 transition-colors mb-4 group/map">
                            ...
                            <div class="bg-[#0a0a0a] p-3 rounded-xl border border-white/5">
                                ...
                                <div class="text-xs font-medium text-gray-300">
                                    @if ($comp->regis_start_date && $comp->regis_end_date)
                                        <span class="text-gray-500 font-normal">เริ่ม</span> {{ \Carbon\Carbon::parse($comp->regis_start_date)->format('d/m/y H:i') }} น.<br>
                                        <span class="text-gray-500 font-normal">ถึง</span> {{ \Carbon\Carbon::parse($comp->regis_end_date)->format('d/m/y H:i') }} น.
                                    @else
                                        <span class="text-gray-500 font-normal">ไม่ได้กำหนด</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="bg-[#0a0a0a] p-3 rounded-xl border border-white/5">
                                ...
                                <div class="text-xs font-medium text-gray-300">
                                    @if ($comp->event_start_date && $comp->event_end_date)
                                        <span class="text-gray-500 font-normal">เริ่ม</span> {{ \Carbon\Carbon::parse($comp->event_start_date)->format('d/m/y') }}<br>
                                        <span class="text-gray-500 font-normal">ถึง</span> {{ \Carbon\Carbon::parse($comp->event_end_date)->format('d/m/y') }}
                                    @else
                                        <span class="text-gray-500 font-normal">ไม่ได้กำหนด</span>
                                    @endif
                                </div>
                        ...
                        <div class="flex items-center justify-between gap-3 pt-4 border-t border-white/5">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button onclick="confirmDelete('{{ route('admin.competitions.destroy', $comp->id) }}', {{ Js::from($comp->name) }})"
                                    class="w-10 h-10 flex items-center justify-center text-red-400 hover:bg-red-500/10 rounded-xl transition-colors border border-transparent hover:border-red-500/20" title="ลบ">
            ...
            @empty
                <div class="col-span-full py-24 flex flex-col items-center justify-center bg-[#121212] border border-white/5 rounded-2xl">
                    <div class="w-20 h-20 bg-[#1a1a1a] rounded-full flex items-center justify-center mb-5 border border-white/5">
                        <i class="fas fa-folder-open text-4xl text-gray-600"></i>
                    </div>
                    <p class="text-lg font-semibold text-gray-300 mb-1">ยังไม่มีรายการแข่งขัน</p>
                    <p class="text-sm font-normal text-gray-500 mb-6">สร้างรายการแรกของคุณเพื่อเริ่มต้นระบบ</p>
                    <button @click="$dispatch('open-modal', 'add-competition'); initMap('map-add', 'add_lat', 'add_lng')"
                        class="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold rounded-xl shadow-md transition-colors flex items-center gap-2">
        ...
        <x-modal name="add-competition" focusable maxWidth="3xl">
            <div class="bg-[#121212] flex flex-col h-[90vh] overflow-hidden rounded-2xl border border-white/10">
                <div class="px-6 py-5 border-b border-white/5 shrink-0 flex items-center justify-between bg-[#0a0a0a] z-20">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-blue-500/10 text-blue-400 rounded-xl flex items-center justify-center">
                            <i class="fas fa-plus-circle text-lg"></i>
                        </div>
                        <h2 class="text-xl font-semibold text-white">สร้างงานแข่งขันใหม่</h2>
                    </div>
                    <button @click="$dispatch('close-modal', 'add-competition')" type="button" class="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-white hover:bg-white/10 rounded-lg transition-colors focus:outline-none">
                ...
                <form method="POST" action="{{ route('admin.competitions.store') }}" enctype="multipart/form-data" class="flex flex-col flex-1 min-h-0">
                    ...
                    <div class="p-6 sm:p-8 overflow-y-auto custom-scrollbar flex-1 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2 space-y-2">
                                <label class="block text-sm font-semibold text-gray-300">ชื่อรายการแข่งขัน <span class="text-red-500">*</span></label>
                                <input type="text" name="name" required placeholder="เช่น Easykids Competitions 2026"
                                    class="w-full px-4 py-3 text-sm border-white/10 bg-[#0f0f0f] text-white rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none placeholder-gray-600">
                            </div>

                            {{-- Location --}}
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-300">สถานที่จัดงาน</label>
                            ...
                            {{-- Status --}}
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-300">สถานะงาน <span class="text-red-500">*</span></label>
                                <div x-data="{ open: false, selected: 'draft', options: { 'draft': 'ร่างงาน (ซ่อนจากหน้าเว็บ)', 'published': 'เผยแพร่ (ระบบเวลาทำงานอัตโนมัติ)', 'cancelled': 'ยกเลิกงานแข่ง' } }" class="relative" @click.outside="open = false">
                                    <button @click="open = !open" type="button" class="w-full px-4 py-3 bg-[#0f0f0f] border border-white/10 rounded-xl text-sm text-left flex items-center justify-between focus:ring-2 focus:ring-blue-500/20 outline-none">
                                        <span x-text="options[selected]" class="text-white"></span>
                                        <i class="fas fa-chevron-down text-xs text-gray-500" :class="open && 'rotate-180'"></i>
                                    </button>
                                    <div x-show="open" style="display:none" class="absolute z-[60] w-full mt-1 bg-[#1a1a1a] shadow-xl rounded-xl border border-white/10 overflow-hidden">
                                        <template x-for="(label, val) in options" :key="val">
                                            <div @click="selected = val; open = false" class="px-4 py-3 text-sm cursor-pointer hover:bg-white/5 text-gray-300" x-text="label"></div>
                                        </template>
                                    </div>
                            ...
                            {{-- Map Segment --}}
                            <div class="md:col-span-2 p-5 bg-[#0a0a0a] rounded-2xl border border-white/5">
                                <div class="flex items-center justify-between mb-3">
                                    <div class="flex items-center">
                                        <i class="fas fa-map-marker-alt text-red-500 mr-2 text-lg"></i>
                                        <label class="block text-sm font-semibold text-gray-300">ปักหมุดสถานที่จัดงาน (ลากหมุดสีน้ำเงิน)</label>
                                    </div>
                                    <div class="flex space-x-3 text-xs text-gray-400 bg-[#121212] px-3 py-1.5 rounded-lg border border-white/10 shadow-sm">
                                        <span>Lat: <span id="add_lat_display" class="font-mono text-blue-400">18.7883</span></span>
                            ...
                            {{-- Description --}}
                            <div class="md:col-span-2 space-y-2">
                                <label class="block text-sm font-semibold text-gray-300">รายละเอียดงาน / คำอธิบาย</label>
                                <textarea name="description" rows="3" placeholder="อธิบายภาพรวมของงาน กติกาคร่าวๆ หรือลิงก์สำคัญ..."
                                    class="w-full px-4 py-3 text-sm border-white/10 bg-[#0f0f0f] text-white rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none placeholder-gray-600"></textarea>
                            </div>

                            {{-- Banner --}}
                            <div class="md:col-span-2 space-y-2">
                                <label class="block text-sm font-semibold text-gray-300">แบนเนอร์งาน (PNG/JPG)</label>
                            ...
                            <div class="md:col-span-2 p-5 bg-[#1a1a1a] rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-5 border border-white/5">
                                <div class="md:col-span-2 flex items-center border-b border-white/5 pb-3">
                                    ...
                                    <span class="text-sm font-semibold text-blue-400">ช่วงเวลาเปิดรับสมัคร</span>
                                </div>
                                <div class="space-y-1.5">
                                    ...
                                    <input type="datetime-local" name="regis_start_date"
                                        class="w-full px-4 py-2.5 text-sm border-white/10 bg-[#0f0f0f] text-white rounded-xl focus:ring-2 focus:ring-blue-500/20 outline-none [color-scheme:dark] cursor-pointer">
                                </div>
                                <div class="space-y-1.5">
                            ...
                            <div class="md:col-span-2 p-5 bg-[#1a1a1a] rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-5 border border-white/5">
                                <div class="md:col-span-2 flex items-center border-b border-white/5 pb-3">
                                    ...
                                    <span class="text-sm font-semibold text-purple-400">วันจัดงานแข่งขันจริง</span>
                                </div>
                                <div class="space-y-1.5">
                    ...
                    {{-- Footer --}}
                    <div class="px-6 py-4 bg-[#0a0a0a] border-t border-white/5 shrink-0 flex justify-end gap-3 z-20">
                        <button type="button" @click="$dispatch('close-modal', 'add-competition')" class="px-6 py-2.5 bg-[#1a1a1a] hover:bg-white/5 border border-white/10 text-gray-300 rounded-xl text-sm font-semibold transition-colors focus:outline-none shadow-sm">ยกเลิก</button>
                        <button type="submit" class="px-8 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-sm font-semibold shadow-md flex items-center gap-2 transition-all">
            ...
            <div class="bg-[#121212] flex flex-col h-[90vh] overflow-hidden rounded-2xl border border-white/10">
                <div class="px-6 py-5 border-b border-white/5 shrink-0 flex items-center justify-between bg-[#0a0a0a] z-20">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-xl font-semibold text-white">แก้ไขข้อมูลงานแข่งขัน</h2>
                    </div>
                    <button @click="$dispatch('close-modal', 'edit-competition')" type="button" class="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-white hover:bg-white/10 rounded-lg transition-colors focus:outline-none">
                        <i class="fas fa-times text-lg"></i>
                    </button>
                </div>
                ...
                <form method="POST" :action="`/admin/competitions/${editComp.id}`" enctype="multipart/form-data" class="flex flex-col flex-1 min-h-0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2 space-y-2">
                                ...
                                <input type="text" name="name" x-model="editComp.name" required
                                    class="w-full px-4 py-3 text-sm border-white/10 bg-[#0f0f0f] text-white rounded-xl focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 transition-all outline-none">
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div x-data="{ open: false, options: { 'draft': 'ร่างงาน (ซ่อนจากหน้าเว็บ)', 'published': 'เผยแพร่ (ระบบเวลาทำงานอัตโนมัติ)', 'cancelled': 'ยกเลิกงานแข่ง' } }" class="relative" @click.outside="open = false">
                                    <button @click="open = !open" type="button" class="w-full px-4 py-3 bg-[#0f0f0f] border border-white/10 rounded-xl text-sm text-left flex items-center justify-between focus:ring-2 focus:ring-yellow-500/20 outline-none">
                                        <span x-text="options[editComp.status] ? options[editComp.status] : (editComp.status === 'registration' || editComp.status === 'ongoing' || editComp.status === 'completed' ? 'เผยแพร่ (ระบบเวลาทำงานอัตโนมัติ)' : editComp.status)" class="text-white"></span>
                                        <i class="fas fa-chevron-down text-xs text-gray-500" :class="open && 'rotate-180'"></i>
                                    </button>
                                    <div x-show="open" style="display:none" class="absolute z-[60] w-full mt-1 bg-[#1a1a1a] shadow-xl rounded-xl border border-white/10 overflow-hidden">
                                        <template x-for="(label, val) in options" :key="val">
                                            <div @click="editComp.status = val; open = false" class="px-4 py-3 text-sm cursor-pointer hover:bg-white/5 text-gray-300" x-text="label"></div>
                                        </template>
                                    </div>
                            ...
                            <div class="md:col-span-2 p-5 bg-[#0a0a0a] rounded-2xl border border-white/5">
                                <div class="flex items-center justify-between mb-3">
                                    <div class="flex items-center">
                                        <i class="fas fa-map-marker-alt text-red-500 mr-2 text-lg"></i>
                                        <label class="block text-sm font-semibold text-gray-300">ปักหมุดสถานที่จัดงาน</label>
                                    </div>
                                    <div class="flex space-x-3 text-xs text-gray-400 bg-[#121212] px-3 py-1.5 rounded-lg border border-white/10 shadow-sm">
                                        <span>Lat: <span id="edit_lat_display" class="font-mono text-yellow-500" x-text="editComp.latitude || '18.7883'"></span></span>
                            ...
                            <div class="md:col-span-2 space-y-2">
                                <label class="block text-sm font-semibold text-gray-300">รายละเอียดงาน / คำอธิบาย</label>
                                <textarea name="description" x-model="editComp.description" rows="3" class="w-full px-4 py-3 text-sm border-white/10 bg-[#0f0f0f] text-white rounded-xl focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 transition-all outline-none"></textarea>
                            </div>

                            {{-- Banner --}}
                            <div class="md:col-span-2 space-y-2">
                                <label class="text-sm font-semibold text-gray-300 flex items-center justify-between">
                                    <span>เปลี่ยนแบนเนอร์ใหม่ <span class="text-gray-500 font-normal ml-1">(ข้ามหากไม่ต้องการเปลี่ยน)</span></span>
                                </label>
                            ...
                            <div class="md:col-span-2 p-5 bg-[#1a1a1a] rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-5 border border-white/5">
                                <div class="md:col-span-2 flex items-center border-b border-white/5 pb-3">
                                    ...
                                    <span class="text-sm font-semibold text-yellow-500">ช่วงเวลาเปิดรับสมัคร</span>
                                </div>
                                <div class="space-y-1.5">
                                    <label class="block text-xs font-semibold text-gray-400">วันเริ่มรับสมัคร</label>
                    ...
                    {{-- Footer --}}
                    <div class="px-6 py-4 bg-[#0a0a0a] border-t border-white/5 shrink-0 flex justify-end gap-3 z-20">
                        <button type="button" @click="$dispatch('close-modal', 'edit-competition')" class="px-6 py-2.5 bg-[#1a1a1a] hover:bg-white/5 border border-white/10 text-gray-300 rounded-xl text-sm font-semibold transition-colors focus:outline-none shadow-sm">ยกเลิก</button>
                        <button type="submit" class="px-8 py-2.5 bg-yellow-600 hover:bg-yellow-500 text-white rounded-xl text-sm font-semibold shadow-md flex items-center gap-2 transition-all">
