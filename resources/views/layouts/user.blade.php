...
<body class="font-sans antialiased bg-[#0a0a0a] text-gray-200 leading-relaxed" x-data="{ mobileSidebarOpen: false }">

    <div class="flex h-screen overflow-hidden">
        {{-- 1. Backdrop สำหรับมือถือ --}}
        <div x-show="mobileSidebarOpen" @click="mobileSidebarOpen = false"
            class="fixed inset-0 z-40 bg-black/70 lg:hidden transition-opacity" style="display: none;"></div>

        {{-- 2. Sidebar --}}
        <aside :class="mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            class="fixed inset-y-0 left-0 z-50 w-72 bg-[#0f0f0f] border-r border-white/5 transition-transform duration-300 transform -translate-x-full lg:static lg:translate-x-0 overflow-y-auto shadow-2xl lg:shadow-none">

            <div class="flex flex-col h-full relative">
                {{-- ปุ่มปิด Sidebar สำหรับมือถือ (ซ่อนในจอใหญ่) --}}
                <button @click="mobileSidebarOpen = false" class="absolute top-4 right-4 text-gray-400 hover:text-white transition-colors bg-white/5 rounded-full p-1.5 lg:hidden">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>

                <div class="px-8 py-8 border-b border-white/5 flex justify-center items-center">
                ...
                <nav class="flex-1 px-4 pt-6 pb-4 space-y-2" @click="if(window.innerWidth < 1024) mobileSidebarOpen = false">
                    <div class="px-4 py-2 text-[10px] font-normal text-gray-500 uppercase tracking-[0.2em]">เมนูหลัก
                    </div>
                    ...
                    <a href="{{ route('user.dashboard') }}"
                        class="flex items-center px-4 py-3 text-sm rounded-xl transition-all duration-300 
                              {{ $isDashboard ? 'text-blue-400 bg-blue-500/10 font-normal' : 'font-normal text-gray-400 hover:text-white hover:bg-white/5' }}">
                    ...
                    <a href="{{ route('user.teams.index') }}"
                        class="flex items-center px-4 py-3 text-sm rounded-xl transition-all duration-300 
                              {{ $isTeams ? 'text-blue-400 bg-blue-500/10 font-normal' : 'font-normal text-gray-400 hover:text-white hover:bg-white/5' }}">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('user.registrations') }}"
                        class="flex items-center justify-between px-4 py-3 text-sm rounded-xl transition-all duration-300 
                              {{ $isRegistrations ? 'text-blue-400 bg-blue-500/10 font-normal' : 'font-normal text-gray-400 hover:text-white hover:bg-white/5' }}">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            <header
                class="flex items-center justify-between px-6 lg:px-10 py-4 bg-[#0f0f0f]/80 backdrop-blur-md border-b border-white/5 sticky top-0 z-30">
                        ...
                        <x-slot name="trigger">
                            <button class="flex items-center space-x-3 focus:outline-none group">
                                <div class="text-right hidden sm:block">
                                    <p
                                        class="text-sm font-normal text-white leading-none group-hover:text-blue-400 transition-colors">
                                        {{ Auth::user()->name }}
                                    </p>
                                ...
                                <div
                                    class="w-10 h-10 rounded-md bg-[#1a1a1a] overflow-hidden ring-2 ring-blue-500/10 group-hover:ring-blue-500/30 transition-all shadow-sm">
                                    @if (Auth::user()->avatar)
                                        ...
                                        @endif
                                    @else
                                        <img src="{{ asset('images/default-avatar.png') }}" class="w-full h-full object-cover opacity-70" alt="{{ Auth::user()->name }}">
                                    @endif
                                </div>
                                <svg class="fill-current h-4 w-4 text-gray-500 group-hover:text-gray-300 transition-colors"
                        ...
                        <x-slot name="content">
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <x-dropdown-link :href="route('logout')"
                                    ...
                                    class="text-red-400 font-normal hover:bg-red-500/10 hover:text-red-300">
                                    {{ __('ออกจากระบบ') }}
                                </x-dropdown-link>
                            </form>
                        </x-slot>
            ...
            <main class="flex-1 flex flex-col overflow-x-hidden overflow-y-auto relative">
                ...
                <footer class="mt-auto py-5 px-6 lg:px-8 border-t border-white/5 bg-transparent">
                    <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                        <p class="text-xs font-normal text-gray-500">&copy; {{ date('Y') }}
                            Easykids Robotics. All rights reserved.</p>
                        <div
                            class="flex items-center space-x-6 text-xs font-normal text-gray-500">
                            <a href="{{ route('privacy.policy') }}"
                                class="hover:text-blue-400 transition-colors">Privacy Policy</a>
                            <a href="{{ route('terms.service') }}"
                                class="hover:text-blue-400 transition-colors">Terms of Service</a>
                            <a href="https://www.easykidsrobotics.com/contact-us/"
                                class="hover:text-blue-400 transition-colors">ติดต่อเรา</a>
                        </div>
                    </div>
                </footer>
            </main>
    ...
    <x-modal name="profile-edit-modal" focusable>
        <div class="bg-[#121212] flex flex-col max-h-[90vh] rounded-2xl overflow-hidden shadow-2xl border border-white/10">

            {{-- Sticky Header --}}
            <div
                class="flex items-center justify-between p-5 sm:p-6 md:p-8 pb-4 border-b border-white/5 shrink-0 z-10 bg-[#121212]">
                <div class="flex items-center gap-3 md:gap-4">
                    <div
                        class="w-10 h-10 sm:w-12 sm:h-12 bg-blue-900/30 rounded-xl flex items-center justify-center text-blue-400 shrink-0">
                ...
                <button @click="$dispatch('close-modal', 'profile-edit-modal')"
                    class="text-gray-500 hover:text-white transition-colors p-2 rounded-xl hover:bg-white/5 focus:outline-none focus:ring-2 focus:ring-gray-700">
                    <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <form id="profile-update-form" method="post" action="{{ route('profile.update') }}" enctype="multipart/form-data"
                ...
                <div class="p-5 sm:p-6 md:p-8 overflow-y-auto custom-scrollbar space-y-6 sm:space-y-8">
                    ...
                    <div class="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/5" 
                        ...
                        {{-- ส่วนแสดงรูปภาพ --}}
                        <div class="w-16 h-16 rounded-full bg-[#1a1a1a] border border-white/10 flex items-center justify-center overflow-hidden shrink-0 shadow-sm">
                    ...
                    <div class="space-y-1.5">
                        ...
                        @error('name')
                            <span class="text-[10px] font-normal text-red-400">{{ $message }}</span>
                        @enderror
                    </div>

                    {{-- 1. ข้อมูลภาษาไทย --}}
                    <div class="p-4 sm:p-5 bg-white/5 rounded-2xl border border-white/5">
                        <h3 class="text-[10px] sm:text-xs font-normal text-blue-400 uppercase tracking-widest mb-3 sm:mb-4 flex items-center gap-2">
                            <span class="w-1.5 h-1.5 bg-blue-500 rounded-sm"></span> ข้อมูลภาษาไทย
                        </h3>
                        ...
                        <div class="grid grid-cols-12 gap-3 sm:gap-4">
                            ...
                            <div class="col-span-5 sm:col-span-3 md:col-span-3 lg:col-span-3 space-y-1.5">
                                ...
                                <div x-data="{ open: false, selected: '{{ old('prefix_th', Auth::user()->prefix_th) }}', placeholder: 'โปรดระบุ' }" class="relative" @click.away="open = false">
                                    ...
                                    <div x-show="open" x-transition.opacity.duration.200ms
                                        class="absolute z-50 min-w-[160px] w-full mt-1 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-lg py-1.5 max-h-56 overflow-y-auto custom-scrollbar"
                                        style="display: none;">
                                        <template x-for="item in ['เด็กชาย', 'เด็กหญิง', 'นาย', 'นางสาว', 'นาง']">
                                            <div @click="selected = item; open = false"
                                                class="px-4 py-3 text-xs sm:text-sm text-gray-300 hover:bg-white/5 cursor-pointer transition-colors"
                                                :class="{
                                                    'bg-blue-500/10 text-blue-400 font-normal': selected === item
                                                }">
                            ...
                            <div class="col-span-12 sm:col-span-5 md:col-span-4 lg:col-span-5 space-y-1.5">
                                <label class="text-[10px] sm:text-xs font-normal text-gray-400">นามสกุล</label>
                                <input name="last_name_th" type="text"
                                    value="{{ old('last_name_th', Auth::user()->last_name_th) }}"
                                    class="w-full px-3 sm:px-4 py-2.5 bg-black border {{ $errors->has('last_name_th') ? 'border-red-500' : 'border-white/10' }} rounded-xl focus:ring-2 focus:ring-blue-500/50 outline-none transition-all text-xs sm:text-sm text-white font-normal">
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- 2. ข้อมูลภาษาอังกฤษ --}}
                    <div class="p-4 sm:p-5 bg-white/5 rounded-2xl border border-white/5">
                        <h3 class="text-[10px] sm:text-xs font-normal text-indigo-400 uppercase tracking-widest mb-3 sm:mb-4 flex items-center gap-2">
                            <span class="w-1.5 h-1.5 bg-indigo-500 rounded-sm"></span> ข้อมูลภาษาอังกฤษ
                        </h3>
                        <div class="grid grid-cols-12 gap-3 sm:gap-4">

                            {{-- Prefix EN --}}
                            <div class="col-span-5 sm:col-span-3 md:col-span-3 lg:col-span-3 space-y-1.5">
                                <label class="text-[10px] sm:text-xs font-normal text-gray-400">Prefix</label>
                                <div x-data="{ open: false, selected: '{{ old('prefix_en', Auth::user()->prefix_en) }}', placeholder: 'Select' }" class="relative" @click.away="open = false">
                                    ...
                                    <button @click="open = !open" type="button"
                                        class="w-full px-3 sm:px-4 py-2.5 bg-black border {{ $errors->has('prefix_en') ? 'border-red-500' : 'border-white/10' }} rounded-xl focus:ring-2 focus:ring-blue-500/50 outline-none transition-all text-xs sm:text-sm text-white flex justify-between items-center text-left font-normal">
                                        <span x-text="selected || placeholder"
                                            :class="{ 'text-gray-500': !selected }"
                                            class="truncate mr-2"></span>
                                        <svg class="w-4 h-4 text-gray-500 transition-transform duration-200 shrink-0"
                            ...
                            {{-- First Name --}}
                            <div class="col-span-7 sm:col-span-4 md:col-span-5 lg:col-span-4 space-y-1.5">
                                <label class="text-[10px] sm:text-xs font-normal text-gray-400">First Name</label>
                                <input name="first_name_en" type="text"
                                    value="{{ old('first_name_en', Auth::user()->first_name_en) }}"
                                    class="w-full px-3 sm:px-4 py-2.5 bg-black border {{ $errors->has('first_name_en') ? 'border-red-500' : 'border-white/10' }} rounded-xl focus:ring-2 focus:ring-blue-500/50 outline-none transition-all text-xs sm:text-sm text-white font-normal">
                            </div>

                            {{-- Last Name --}}
                            <div class="col-span-12 sm:col-span-5 md:col-span-4 lg:col-span-5 space-y-1.5">
                                <label class="text-[10px] sm:text-xs font-normal text-gray-400">Last Name</label>
                                <input name="last_name_en" type="text"
                                    value="{{ old('last_name_en', Auth::user()->last_name_en) }}"
                                    class="w-full px-3 sm:px-4 py-2.5 bg-black border {{ $errors->has('last_name_en') ? 'border-red-500' : 'border-white/10' }} rounded-xl focus:ring-2 focus:ring-blue-500/50 outline-none transition-all text-xs sm:text-sm text-white font-normal">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-5 pt-2">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input name="phone_number" type="text"
                                ...
                                class="w-full px-4 py-2.5 bg-black border {{ $errors->has('phone_number') ? 'border-red-500' : 'border-white/10' }} rounded-xl focus:ring-2 focus:ring-blue-500/50 outline-none transition-all text-sm text-white font-normal placeholder-gray-600">
                        </div>
                        ...
                        <div class="sm:col-span-2 md:col-span-1 space-y-1.5">
                            ...
                            <div x-data="{ open: false, selected: '{{ old('shirt_size', Auth::user()->shirt_size) }}', placeholder: 'โปรดระบุขนาด' }" class="relative" @click.away="open = false">
                                ...
                                <button @click="open = !open" type="button"
                                    ...
                                    <span x-text="selected || placeholder"
                                        :class="{ 'text-gray-500': !selected }"></span>
                                ...
                                <div x-show="open" x-transition.opacity.duration.200ms
                                    class="absolute bottom-full mb-1 z-50 min-w-[160px] w-full bg-[#1a1a1a] border border-white/10 rounded-xl shadow-lg py-1.5 max-h-56 overflow-y-auto custom-scrollbar"
                                    style="display: none;">
                                    <template x-for="item in ['S', 'M', 'L', 'XL', '2XL', '3XL']">
                                        <div @click="selected = item; open = false"
                                            class="px-4 py-3 text-sm text-gray-300 hover:bg-white/5 cursor-pointer transition-colors"
                                            :class="{
                                                'bg-blue-500/10 text-blue-400 font-normal': selected === item
                                            }">
                                            <span x-text="item"></span>
                    ...
                    <div class="p-3 sm:p-4 bg-white/5 rounded-xl border border-dashed border-white/10 mt-4">
                        <label class="text-[10px] sm:text-xs font-normal text-gray-500 flex items-center gap-1.5 mb-1">
                ...
                <div class="p-5 sm:p-6 md:p-8 pt-4 border-t border-white/5 bg-[#121212] shrink-0 z-10">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const ToastConfig = {
                background: '#1a1a1a', // ปรับเป็นสีเข้ม
                color: '#ffffff', // ให้ตัวหนังสือหลักเป็นสีขาว
                confirmButtonColor: '#3b82f6',
                customClass: {
                    popup: 'rounded-2xl p-4 shadow-sm border border-white/10',
                    title: 'text-xl font-normal text-white',
                    htmlContainer: 'text-left text-gray-300',
            ...
            <div class="mt-4 space-y-3">
                @foreach ($errors->all() as $error)
                    <div class="flex items-center gap-3 p-3 bg-amber-500/10 rounded-lg border-l-4 border-amber-500">
                        <i class="fas fa-info-circle text-amber-500 text-xs"></i>
                        <span class="text-xs text-gray-300 font-normal leading-tight">{!! addslashes($error) !!}</span>
                    </div>
                @endforeach
            </div>
        `;
