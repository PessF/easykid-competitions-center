<x-user-layout>
    ...
    @php
        ...
        // 🚀 เปลี่ยนสี Badge เป็นสไตล์ Modern Dark Mode (โปร่งแสง มีขอบ)
        $statusConfig = [
            'draft' => [
    ...
    <div x-data="{
        ...
        <div class="relative w-full rounded-2xl overflow-hidden bg-[#080808] mb-6"
            ...
            <div
                class="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-[#0a0a0a] to-transparent z-20">
            </div>

            <div class="relative z-20 h-full flex flex-col justify-end px-8 sm:px-12 pb-9">
                <div class="flex items-center gap-2.5 mb-3">
                    <div class="w-5 h-px bg-blue-500"></div>
                    <span class="text-blue-500 text-[10px] font-normal uppercase tracking-[0.2em]">Easykids
                        Competition</span>
                </div>
        ...
        <div class="sticky top-16 z-40 mb-6">
            <div class="bg-[#111]/95 backdrop-blur-xl border border-white/5 rounded-2xl shadow-sm flex items-center gap-3 px-5"
                style="height: 52px;">
                ...
                <button x-show="search !== ''" @click="search = ''" x-cloak type="button"
                    class="w-6 h-6 flex items-center justify-center text-gray-500 hover:text-gray-300 transition-colors focus:outline-none shrink-0 mr-1">
                ...
                <div x-data="{
                    ...
                    <button @click="open = !open" type="button"
                        class="h-full flex items-center gap-2 pl-3 pr-2 bg-transparent border-none focus:ring-0 outline-none text-sm text-gray-400 font-normal transition-colors hover:text-blue-400">
                        <span x-text="options[filterStatus]"></span>
                        <i class="fas fa-chevron-down text-[10px] text-gray-500 transition-transform duration-200"
                    ...
                    <div x-show="open" x-transition.opacity.duration.200ms style="display: none;"
                        ...
                        <template x-for="(label, val) in options" :key="val">
                            <button @click="filterStatus = val; open = false" type="button"
                                class="w-full text-left px-4 py-2.5 text-sm transition-colors flex items-center justify-between group font-normal"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @forelse ($competitions as $comp)
                <div x-show="isVisible({{ $comp->id }})"
                    class="group bg-[#141414] border border-white/5 rounded-2xl overflow-hidden hover:border-blue-500/30 hover:shadow-[0_8px_30px_rgb(0,0,0,0.5)] transition-all duration-300 flex flex-col">

                    {{-- ── IMAGE ── --}}
                    <div class="aspect-[21/9] relative overflow-hidden shrink-0 bg-[#111] border-b border-white/5">
                        ...
                        @endif
                        <div class="absolute inset-0 bg-gradient-to-t from-[#141414]/90 via-black/20 to-transparent"></div>

                        {{-- Status badge --}}
                        <div class="absolute top-4 left-4">
                            <span
                                class="{{ $comp->ui_config['bg'] }} inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-normal uppercase rounded-lg tracking-wide border shadow-lg border-white/20">
                        ...
                        @if ($comp->time_badge->show)
                            <div
                                class="absolute top-4 right-4 flex items-center gap-1 bg-black/70 backdrop-blur-md border px-2.5 py-1 rounded-lg {{ $comp->time_badge->urgent ? 'border-red-500/50 animate-pulse' : 'border-white/10' }}">
                                <i
                                    class="fas {{ $comp->time_badge->urgent ? 'fa-exclamation-circle text-red-400' : 'fa-clock text-amber-400' }} text-[9px]"></i>
                                <span
                                    class="text-[10px] font-normal {{ $comp->time_badge->urgent ? 'text-red-400' : 'text-amber-400' }}">{{ $comp->time_badge->text }}</span>
                            </div>
                        @endif
                        ...
                        <div class="absolute bottom-4 left-4">
                            <div
                                class="bg-[#1a1a1a] border border-white/10 rounded-xl px-3 py-2 text-center shadow-lg min-w-[3rem]">
                                @if ($comp->event_start_date)
                                    <span class="block text-xl font-normal leading-none text-white">
                                        {{ \Carbon\Carbon::parse($comp->event_start_date)->format('d') }}
                                    </span>
                                    <span
                                        class="block text-[9px] font-normal uppercase text-blue-400 tracking-widest mt-0.5">
                                        {{ \Carbon\Carbon::parse($comp->event_start_date)->translatedFormat('M') }}
                                    </span>
                                @else
                                    <span class="block text-lg font-normal leading-none text-gray-500">TBA</span>
                                    <span
                                        class="block text-[9px] font-normal uppercase text-gray-600 tracking-widest mt-0.5">รอประกาศ</span>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex-1 flex flex-col p-6 min-w-0">
                        ...
                        {{-- Title --}}
                        <h3
                            class="text-lg font-normal text-white leading-snug group-hover:text-blue-400 transition-colors line-clamp-2 mb-4 drop-shadow-md">
                            {{ $comp->name }}
                        </h3>
                        ...
                        <div class="flex flex-col gap-2.5 mb-4 mt-auto">
                            <div
                                class="flex items-center gap-2.5 w-full px-3.5 py-2 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                                <i
                                    class="fas fa-pencil-alt text-emerald-500 text-[10px] w-4 text-center shrink-0"></i>
                            ...
                            <div
                                class="flex items-center gap-2.5 w-full px-3.5 py-2 rounded-xl bg-blue-500/5 border border-blue-500/10">
                                <i
                                    class="fas fa-flag-checkered text-blue-500 text-[10px] w-4 text-center shrink-0"></i>
                                <div class="min-w-0 flex items-center gap-2">
                                    <p
                        ...
                        <div class="pt-4 border-t border-white/5 mt-2">
                            ...
                            @else
                                <a href="{{ route('user.competitions.show', $comp->id) }}"
                                    class="group/btn w-full inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-normal transition-all duration-200 focus:outline-none
                                        @if (in_array($comp->dynamic_status, ['open', 'coming_soon'])) bg-blue-600 text-white hover:bg-blue-500 shadow-md hover:shadow-lg hover:shadow-blue-500/20
                                        @else
            ...
            @empty
                ...
                <div
                    class="col-span-full py-24 flex flex-col items-center justify-center bg-[#141414] border border-white/5 rounded-2xl shadow-sm">
                    <div
                        class="w-16 h-16 bg-[#1a1a1a] rounded-full flex items-center justify-center mb-4 border border-white/5">
        ...
        @if ($competitions->isNotEmpty())
            <div x-show="visibleCount === 0" x-transition:enter="transition ease-out duration-150"
                ...
                class="mt-4 py-20 text-center bg-[#141414] border border-white/5 rounded-2xl shadow-sm">
                <div
                    class="w-14 h-14 bg-[#1a1a1a] border border-white/5 rounded-2xl flex items-center justify-center mx-auto mb-4">
