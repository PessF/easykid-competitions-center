<x-user-layout>
    <main class="flex-1 flex flex-col w-full px-4 sm:px-6 lg:px-8 py-8 mx-auto max-w-7xl font-kanit"
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <div>
                <div class="flex items-center gap-3 mb-1">
                    ...
                    <h1 class="text-2xl sm:text-3xl font-semibold text-gray-900 dark:text-white tracking-tight">จัดการทีมของฉัน</h1>
                </div>
        ...
        {{-- ===== 🚀 UPGRADED TEAM CARDS GRID ===== --}}
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($teams as $team)
                <div class="group relative bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-800 rounded-3xl overflow-hidden hover:border-blue-400 dark:hover:border-blue-800 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 flex flex-col h-full z-0">

                    {{-- Card Background Pattern/Gradient --}}
                    <div class="absolute top-0 inset-x-0 h-32 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/10 dark:to-indigo-900/10 opacity-60 group-hover:opacity-100 transition-opacity duration-500 -z-10"></div>

                    <div class="p-6 sm:p-7 flex flex-col flex-1 relative z-10">
                        
                        {{-- Card Header: Icon & Member Badge --}}
                        <div class="flex items-start justify-between mb-5">
                            <div class="w-14 h-14 bg-white dark:bg-[#111] border border-gray-100 dark:border-gray-700 rounded-2xl flex items-center justify-center shadow-sm shrink-0 group-hover:scale-110 transition-transform duration-300">
                                <i class="fas fa-shield-alt text-blue-600 dark:text-blue-400 text-2xl"></i>
                            </div>
                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/80 dark:bg-[#111]/80 backdrop-blur-sm border border-gray-100 dark:border-gray-700 text-gray-700 dark:text-gray-300 text-xs font-semibold rounded-xl shadow-sm">
                                <i class="fas fa-users text-blue-500"></i> สมาชิก {{ $team->members_count }} คน
                            </span>
                        </div>

                        {{-- Text Content --}}
                        <div class="mb-5">
                            <h3 class="text-xl font-bold text-gray-900 dark:text-white leading-snug line-clamp-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors" title="{{ $team->name }}">
                                {{ $team->name }}
                            </h3>
                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400 flex items-center gap-2 mt-2.5">
                                <i class="fas fa-school text-gray-400"></i>
                                <span class="truncate">{{ $team->school_name }}</span>
                            </p>
                        </div>

                        {{-- Avatar Overlaps --}}
                        <div class="mt-auto pt-5 pb-1 border-t border-gray-100 dark:border-gray-800/80">
                            <div class="flex -space-x-3">
                                @for($i = 0; $i < min($team->members_count, 4); $i++)
                                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 border-2 border-white dark:border-[#1a1a1a] flex items-center justify-center shadow-sm relative z-[{{ 4 - $i }}]">
                                        <i class="fas fa-user text-xs text-gray-400 dark:text-gray-500"></i>
                                    </div>
                                @endfor
                                @if($team->members_count > 4)
                                    <div class="w-10 h-10 rounded-full bg-blue-50 dark:bg-blue-900/30 border-2 border-white dark:border-[#1a1a1a] flex items-center justify-center shadow-sm relative z-0">
                                        <span class="text-xs font-bold text-blue-600 dark:text-blue-400">+{{ $team->members_count - 4 }}</span>
                                    </div>
                                @endif
                            </div>
                        </div>

                        {{-- 🚀 Upgraded Action Buttons (Grid Layout) --}}
                        <div class="grid grid-cols-4 gap-3 mt-6">
                            <button @click='openEditModal(@json($team->load("members")))'
                                    class="col-span-3 flex items-center justify-center gap-2 py-3 text-sm font-semibold text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-600 dark:hover:text-white rounded-xl transition-all duration-200 group/edit border border-blue-100 dark:border-blue-900/30 hover:border-transparent">
                                <i class="fas fa-pen text-xs group-hover/edit:rotate-12 transition-transform"></i>
                                จัดการและแก้ไขทีม
                            </button>
                            ...
                            <form action="{{ route('user.teams.destroy', $team->id) }}" method="POST"
                                ...
                                @csrf @method('DELETE')
                                <button type="submit"
                                        class="w-full h-full flex items-center justify-center text-red-500 bg-red-50 dark:bg-red-900/10 hover:bg-red-500 hover:text-white rounded-xl transition-all duration-200 border border-red-100 dark:border-red-900/20 hover:border-transparent"
                                        title="ลบทีม">
            ...
            @empty
                <div class="col-span-full">
                    <div class="flex flex-col items-center justify-center py-24 px-8 border-2 border-dashed border-gray-300 dark:border-gray-800 rounded-3xl bg-white/50 dark:bg-[#1a1a1a]/50 text-center">
                        <div class="w-24 h-24 bg-blue-50 dark:bg-blue-900/20 rounded-full flex items-center justify-center mb-6">
                            <i class="fas fa-users-slash text-4xl text-blue-300 dark:text-blue-600"></i>
                        </div>
                        <p class="text-xl font-bold text-gray-800 dark:text-gray-200 mb-2">ยังไม่มีทีมในระบบ</p>
                        <p class="text-base font-normal text-gray-500 dark:text-gray-400 mb-8">สร้างทีมแรกของคุณเพื่อเตรียมพร้อมสำหรับการแข่งขัน</p>
                        <button @click="openCreateModal()"
                                class="inline-flex items-center gap-2 px-8 py-3.5 bg-blue-600 hover:bg-blue-700 text-white text-base font-medium rounded-xl transition-colors shadow-md">
        ...
        <x-modal name="team-form-modal" maxWidth="4xl" focusable>
            <div class="max-h-[90vh] overflow-y-auto custom-scrollbar bg-gray-50 dark:bg-[#0a0a0a] rounded-2xl flex flex-col relative w-full">
                ...
                <form :action="formAction" method="POST" class="flex flex-col flex-1">
                    ...
                    <div class="p-4 sm:p-6 space-y-6 flex-1">
                        ...
                        <div class="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
                            ...
                            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2">
                                    <label class="block text-base font-medium text-gray-700 dark:text-gray-300">ชื่อทีม <span class="text-red-500">*</span></label>
                                ...
                                <div class="space-y-2">
                                    <label class="block text-base font-medium text-gray-700 dark:text-gray-300">โรงเรียน/สถาบัน <span class="text-red-500">*</span></label>
                        ...
                        <div class="space-y-5">
                            <div class="flex items-center justify-between px-2 pt-2">
                                ...
                                <span class="text-sm font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-3.5 py-1.5 rounded-lg border border-blue-100 dark:border-blue-800/50"
                            ...
                            <template x-for="(member, index) in members" :key="member.id">
                                <div class="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm relative group animate-fade-in">
                                    ...
                                    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-white/[0.02]">
                                        <div class="flex items-center gap-3.5">
                                            <div class="w-9 h-9 rounded-lg bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center">
                                                <span class="text-base font-bold text-blue-600 dark:text-blue-400" x-text="index + 1"></span>
                                            </div>
                                            <span class="text-lg font-medium text-gray-800 dark:text-gray-200">ลูกทีมคนที่ <span x-text="index + 1"></span></span>
                                        </div>
                                    ...
                                    <div class="p-6 space-y-7">
                                        ...
                                        {{-- Row 1: TH Name --}}
                                        <div>
                                            <p class="text-sm font-medium text-blue-600 dark:text-blue-400 mb-3.5 bg-blue-50 dark:bg-blue-900/20 inline-block px-3 py-1 rounded-lg">
                                                <i class="fas fa-language mr-1.5"></i> ภาษาไทย
                                            </p>
                                            <div class="grid grid-cols-12 gap-5">
                                                <div class="col-span-12 md:col-span-3">
                                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">คำนำหน้า <span class="text-red-500">*</span></label>
                                        ...
                                        {{-- Row 2: EN Name --}}
                                        <div>
                                            <p class="text-sm font-medium text-indigo-600 dark:text-indigo-400 mb-3.5 bg-indigo-50 dark:bg-indigo-900/20 inline-block px-3 py-1 rounded-lg">
                                                <i class="fas fa-globe-americas mr-1.5"></i> English
                                            </p>
                                            <div class="grid grid-cols-12 gap-5">
                                                <div class="col-span-12 md:col-span-3">
                                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Prefix <span class="text-red-500">*</span></label>
                                                    <div x-data="{ open: false }" class="relative" @click.outside="open = false">
                                                        <button @click="open = !open" type="button" class="w-full px-4 py-3.5 bg-white dark:bg-black/20 border border-gray-300 dark:border-gray-700 rounded-xl text-base font-normal text-left flex items-center justify-between focus:ring-2 focus:ring-blue-500/20 outline-none">
                                            ...
                                            <div class="grid grid-cols-12 gap-5">
                                                <div class="col-span-12 sm:col-span-6">
                                                    ...
                                                    <div class="flex justify-between items-end mb-2">
                                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">วันเกิด (ปี ค.ศ.) <span class="text-red-500">*</span></label>
                                                        <span x-show="member.birth_date" x-transition.opacity 
                                                              class="text-xs font-semibold text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 px-2.5 py-1 rounded-md border border-green-200 dark:border-green-800/30 shadow-sm" style="display: none;">
                                                ...
                                                <div class="col-span-12 sm:col-span-6">
                                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">ไซส์เสื้อ (Shirt Size) <span class="text-red-500">*</span></label>
                                                    <div x-data="{ open: false }" class="relative" @click.outside="open = false">
                                                        <button @click="open = !open" type="button" class="w-full px-4 py-3.5 bg-white dark:bg-black/20 border border-gray-300 dark:border-gray-700 rounded-xl text-base font-normal text-left flex items-center justify-between focus:ring-2 focus:ring-blue-500/20 outline-none">
                                                        ...
                                                        <div x-show="open" x-transition.opacity style="display:none" class="absolute z-[60] w-full mt-1 bg-white dark:bg-[#222] shadow-xl rounded-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                                                            <div class="grid grid-cols-3 gap-2 p-3">
                                                                <template x-for="opt in ['JS', 'JM', 'JL', 'S', 'M', 'L', 'XL', '2XL', '3XL']">
                                                                    <div @click="member.shirt_size = opt; open = false" class="text-center py-2.5 text-base font-medium rounded-xl cursor-pointer transition-colors" :class="member.shirt_size === opt ? 'bg-blue-600 text-white shadow-sm' : 'bg-gray-50 dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'" x-text="opt"></div>
                                                                </template>
                                                            </div>
                                                        </div>
                                                        <input type="hidden" :name="`members[${index}][shirt_size]`" x-model="member.shirt_size">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="flex flex-col-reverse sm:flex-row items-center justify-between gap-4 px-6 py-5 border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-[#111] sticky bottom-0 z-50 shrink-0">
                        <p class="text-sm font-normal text-gray-500 dark:text-gray-400 w-full sm:w-auto text-center sm:text-left">
                            <i class="fas fa-asterisk text-red-400 mr-1.5"></i> กรุณาตรวจสอบข้อมูลให้ครบถ้วนก่อนบันทึก เพราะเราถือว่าคุณตรวจเช็คถี่ถ้วนแล้ว
                        </p>
            ...
            Alpine.data('teamManager', () => ({
                ...
                getEmptyMember() {
                    return {
                        id: Date.now(),
                        member_id: null,
                        prefix_th: 'เด็กชาย', first_name_th: '', last_name_th: '',
                        prefix_en: 'Master', first_name_en: '', last_name_en: '',
                        birth_date: '', shirt_size: 'M'
                    };
                },
                ...
                openCreateModal() {
                    ...
                    this.teamData = { name: '', school_name: '' };
                ...
                openEditModal(team) {
                    ...
                    this.teamData = { name: team.name, school_name: team.school_name };
                    
                    if (team.members && team.members.length > 0) {
                        this.members = team.members.map(m => ({
                            id: m.id,
                            member_id: m.id,
                            prefix_th: m.prefix_th, first_name_th: m.first_name_th, last_name_th: m.last_name_th,
                            prefix_en: m.prefix_en, first_name_en: m.first_name_en, last_name_en: m.last_name_en,
